...
var module=angular.module("lightGrid", []).constant("DEFAULT_VIEW", "read");
...
module.provider("lgLocalDataProviderFactory", function () {
	var self = this;
...
module.provider("lgServerDataProviderFactory", function () {
	var self = this;
	//延迟放止页面抖动
...
module.directive("lgGrid", function gridDirective() {
	"use strict";
...
module.directive("lgRow", ["$parse", "$animate", "DEFAULT_VIEW", function rowDirective($parse, $animate, DEFAULT_VIEW) {
	"use strict";
 ...
 */
module.directive("lgView", ["$compile", function ($compile) {
	"use strict";
...
module.directive("lgExpandedRow", ["$animate", function ($animate) {
	"use strict";
...
module.directive("lgPager", function () {
	"use strict";
	...
	return {
		...
		replace:true,
		template: 
			"<div class='ngFooterPanel clearfix' >"+
						/*"<div class='fr ngTotalSelectContainer'>" +
							"<span class='fl mr-lg' ng-model='totalItemCount'>共{{ totalItemCount }}条</span>" +
							"<span class='fl mr-sm'>每页:</span>"+
							"<select ng-options='size for size in pageSizes' ng-model='pageSize' class='fl'></select> " +
						"</div>"+*/
						"<div class='fr ngPagerContainer mr-lg'>"+
							"<button ng-disabled='isFirst' class='ngPagerButton fa_brColor' ng-click='goToFirst()' title='首页'><div class='fa fa-fast-backward fa_color'></div></button>" +
							"<button ng-disabled='isFirst' class='ngPagerButton fa_brColor' ng-click='goToPrevious()'><div class='fa fa-step-backward fa_color'></div></button>" +
							"<input type='text' ng-model='jumpPageNum' ng-keyup='jumpToPage($event)' class='text-center jumpNumber fa_color fa_brColor'/>"+
							"<button ng-disabled='isLast' class='ngPagerButton fa_brColor' ng-click='goToNext()'><div class='fa fa-step-forward fa_color'></div></button>" +
							"<button ng-disabled='isLast' class='ngPagerButton fa_brColor' ng-click='goToLast()'><div class='fa fa-fast-forward fa_color'></div></button>" +
				         "</div>"+
			"</div>",
		...
		template: 
			"<div class='ngFooterPanel clearfix' >"+
						/*"<div class='fr ngTotalSelectContainer'>" +
							...
							"<select ng-options='size for size in pageSizes' ng-model='pageSize' class='fl'></select> " +
						"</div>"+*/
						"<div class='fr ngPagerContainer fa_color'>"+
							"<button ng-disabled='isFirst' class='ngPagerButton fa_brColor fa_bg' ng-click='goToFirst()' title='首页'><div class='fa fa-step-backward fa_color'></div></button>" +
							"<button ng-disabled='isFirst' class='ngPagerButton fa_brColor fa_bg' ng-click='goToPrevious()'><div class='ti-angle-left fa_color'></div></button>" +
							"第&nbsp;<input type='text' ng-model='jumpPageNum' ng-keyup='jumpToPage($event)' class='text-center fa_brColor fa_bg' style='width:.45rem' />&nbsp;页"+
							" / 共&nbsp<span class='ngGridMaxPagesNumber fa_color'>{{pageCount}}</span>&nbsp;页"+
							"<button ng-disabled='isLast' class='ngPagerButton fa_brColor fa_bg' ng-click='goToNext()'><div class='ti-angle-right fa_color'></div></button>" +
							"<button ng-disabled='isLast' class='ngPagerButton fa_brColor fa_bg' ng-click='goToLast()'><div class='fa fa-step-forward fa_color'></div></button>" +
				         "</div>"+
			"</div>",
		link: function pagerLink($scope) {
			$scope.$parent.sum = 0;
 ...
 */
module.directive("lgPersistData", ["$q", function ($q) {
	"use strict";
	return {
		restrict: "A",
 ...
 */
module.directive("lgSorter", ["$timeout", function ($timeout) {
	"use strict";
	return {
 ...
 */
module.directive("lgSwitchView", ["$timeout", function ($timeout) {
	"use strict";
 ...
 */
module.directive("lgToggleExpandedRow", ["$timeout", function ($timeout) {
	"use strict";
