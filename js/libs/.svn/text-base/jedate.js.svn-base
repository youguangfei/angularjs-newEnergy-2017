...
;(function(root, factory) {
    ...
    jet.returnDate = function(obj, format, dateval) {
        ...
        var ye=narr[0]+parseInt(sparr[0]||00);
        var mo=narr[1]+parseInt(sparr[1]||00);
        var mm=parseInt(narr[1])-1;
        
        var setdate = new Date(narr[0]+parseInt(sparr[0]||00), (parseInt(narr[1])-1)+parseInt(sparr[1]||00), narr[2]+parseInt(sparr[2]||00), narr[3]+parseInt(sparr[3]||00), narr[4]+parseInt(sparr[4]||00), narr[5]+parseInt(sparr[4]||00)),
    ...
    jedfn.generateHtml = function(opts,boxCell){
        ...
        var that = this, objCell = that.valCell, weekHtml = "", tmsArr,
            ...
            showTodaybtn=opts.showTodaybtn==false?false:true;
        ...
        //是否显示今天按钮
        (isYYMM || isYY || subhh || !showTodaybtn) ? botCell.find(".jedatetodaymonth").hide() : (jet.isBool(opts.isToday) ? "" : botCell.find(".jedatetodaymonth").hide());
    ...
    jedfn.chooseDays = function(opts,boxCell) {
        ...
        if(opts.festival && lang.name == "cn") {
            ...
            boxCell.find(".jedaul li").on("mouseover", function () {
                ...
                tipDiv.css({"z-index":  (opts.zIndex == undefined ? 20099 + 5 : opts.zIndex + 5),top:tipPos.top,left:tipPos.left,position:"absolute",display:"block"});
    ...
    jet.initDates = function(num, format) {
		...
		var rday=jet.parse([ years, jet.digit(months), jet.digit(days) ], [ jet.digit(hh), jet.digit(mm), jet.digit(ss) ], format);
		return rday;
	};
